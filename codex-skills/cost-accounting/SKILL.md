...
description: |
  当用户需要把产品价格"拆解"成小额日常消费来降低价格抵触时调用此 skill。适用场景：耐用消费品（家电、家具、课程）消费者觉得一次性支出太高、节能型产品需要展示长期节省、增量产品需要让额外花费显得微不足道。不适用于：低价一次性消费品（无需拆解）、冲动消费品类（算账反而破坏氛围）。关键 trigger 信号：用户提到"嫌贵""怎么降低价格感知""每天才多少钱""长期划算"。两种核心方法：平摊法（高价品除以使用天数）和省钱法（算出长期使用节省的金额）。
source_book: 《爆款文案》 关健明
source_chapter: 第三章 3.2
